...
/*   Updated: 2025/12/14 21:39:03 by eturini          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*stash[__FD_SETSIZE] = {0};
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	line = read_new_line(fd, &(stash[fd]));
	return (line);
}
...
int main(){
	...
	printf("max number of file descriptor: %d\n", __FD_SETSIZE);
	if ((fd = open("files/alternate_line_nl_no_nl", O_RDONLY)) == -1)
		return -1;
